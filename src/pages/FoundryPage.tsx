...
export function FoundryPage() {
  ...
  return (
    <div className="grid gap-6">
      <Panel title={t("webhookSettings")} eyebrow={t("mvpBridge")} badge={<CapabilityBadge mode={foundry.connection === "connected" ? "real" : "mixed"} label={foundry.connection} />}>
        <div className="grid gap-4">
          <div className="grid grid-cols-3 gap-3 max-xl:grid-cols-2 max-md:grid-cols-1">
            <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">WebSocket</p>
              <p className="mt-2 break-all font-mono text-sm text-signal-cyan">{foundry.endpoint}</p>
            </div>
            <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
          ...
          <label className="flex items-center gap-3 rounded-lg border border-white/10 bg-panel-950 px-4 py-3 font-bold text-slate-200">
            <input type="checkbox" checked={foundry.enabled} onChange={(event) => updateFoundry({ enabled: event.target.checked })} />
            {t("enableFoundry")}
          </label>
      ...
      <Panel title={t("mockTableButton")} eyebrow={t("esp32InputSimulation")} badge={<CapabilityBadge mode="mock" />}>
