...
export function RfidPage() {
  ...
  return (
    <div className="grid grid-cols-[0.85fr_1.15fr] gap-6 max-xl:grid-cols-1">
      <Panel title={t("tagBinding")} eyebrow={t("rfidWorkflow")} badge={<CapabilityBadge mode="mock" />}>
        <div className="grid gap-4">
          ...
          <p className="text-sm text-slate-400">{t("mockRfidHelp")}</p>
          <div className="rounded-lg border border-white/10 bg-panel-950 p-4">
                  ...
                  <div className="absolute z-20 mt-2 max-h-64 w-full overflow-y-auto rounded-lg border border-white/10 bg-panel-900 p-2 shadow-2xl">
                    {filteredSavedPlayers.length === 0 ? (
                      ...
                      filteredSavedPlayers.map((player) => (
                        <button
                          key={player.id}
                          className={`grid w-full grid-cols-[1fr_auto] items-center gap-3 rounded-lg px-3 py-2 text-left transition ${
            ...
            {selectedSavedPlayer ? (
              <div className="mt-4 flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/[0.035] px-4 py-3">
                <div className="min-w-0">
      ...
      <Panel title={t("liveScanLog")} eyebrow={t("lastEvents")} badge={<CapabilityBadge mode="mock" />}>
        <div className="grid gap-3">
          {events.map((event) => {
            const player = players.find((item) => item.id === event.playerId) ?? playerDatabase.find((item) => item.id === event.playerId);
            return (
              <div key={event.id} className="grid grid-cols-[110px_1fr_110px] items-center gap-4 rounded-lg border border-white/10 bg-white/[0.035] p-4">
                <span className="text-xs font-bold text-slate-500">{formatTime(event.timestamp)}</span>
                <div>
                  <p className="font-mono font-bold text-white">{event.tagId}</p>
